...
public class Exercice3 
{
	public static void main(String[] a)
	{
		...
		/* Ouverture Accueil */
		Exercice3.ouvrirAccueil("index.html");
		}
		...
		{
			trieListe(lstEtu);
			PrintWriter pw = new PrintWriter("page1.html", "UTF-8");
			pw.println(
				"<!DOCTYPE html>\n" +
				"<html lang=\"fr\">\n" +
				"	<head>\n" +
		...
		try
		{
			PrintWriter pw = new PrintWriter("page2.html", "UTF-8");
			pw.println(
				"<!DOCTYPE html>\n" +
				"<html lang=\"fr\">\n" +
				"	<head>\n" +
				...
				pw.println("			<div>");
					...
					if ((((((lstEquipe.size() + 0.0)/ 3) > nbEquipeParColonne && 
					       ((lstEquipe.size() + 0.0)/ 3) >= nbEquipeParColonne + 0.5) && cpt1 == 1) ||
					     ((((lstEquipe.size() + 0.0)/ 3) > nbEquipeParColonne &&
					     ((((lstEquipe.size() + 0.0)/ 3) < nbEquipeParColonne + 0.5) ||
					       ((lstEquipe.size() + 0.0)/ 3) > nbEquipeParColonne)) && cpt1 == 2 )) &&
					         cpt2 == nbEquipeParColonne - 1 )
					{
						nbEquipeActu++;
		...
		try
		{
			PrintWriter pw = new PrintWriter("page3.html", "UTF-8");
			pw.println(
				"<!DOCTYPE html>\n" +
				"<html lang=\"fr\">\n" +
				"	<head>\n" +
		...
		{
			trieListe(lstEtu);
			PrintWriter pw = new PrintWriter("header.css", "UTF-8");
			pw.println(
			"header{\n" +
			"	font-size: 20px;\n" +
		...
		{
			trieListe(lstEtu);
			PrintWriter pw = new PrintWriter("page1.css", "UTF-8");
			pw.println(
			"#titre{\n" +
			"	margin-top: 35px;\n" +
			"	margin-bottom: 30px;\n" +
	...
	try
		{
			PrintWriter pw = new PrintWriter("page2.css", "UTF-8");
			pw.println(
				"#titre{\n" +
				"	border: 1px solid black;\n" +
				"	text-align: center;\n" +
				...
				{
					if ((nbEquipeActu >= (lstEquipe.size() - nbEquipeParColonne * 3 - 1)) && (nbEquipeActu < (lstEquipe.size() - nbEquipeParColonne * 2)))
						pw.println(
							"\n#equipe" + nbEquipeActu + "{\n" +
	...
	try
		{
		PrintWriter pw = new PrintWriter("page3.css", "UTF-8");
		pw.println(
			"#titre{\n" +
			"	border: 1px solid black;\n" +
			"	text-align: center;\n" +
		...
		try
		{
			PrintWriter pw = new PrintWriter("page_acc.css", "UTF-8");
			pw.println(
				"#titre{\n" +
				"	border: 1px solid black;\n" +
				"	text-align: center;\n" +
	...
	public static void trieListe(List<Etudiant> lstEtu)
	{
		Etudiant[] tabEtu = new Etudiant[lstEtu.size()];

		for (int cpt = 0; cpt < lstEtu.size(); cpt++) 
		{
			tabEtu[cpt] = lstEtu.get(cpt);	
		}

		Arrays.sort(tabEtu);

		for (int cpt = 0; cpt < tabEtu.length; cpt++) 
		{
			lstEtu.set(cpt, tabEtu[cpt]);	
		}
	}
	 ...
	 public static void creationEquipe(List<Etudiant> lstEtu, List<List<Integer>> lstInfoSalle, int nbEtuEquipe, boolean bTriCategori, List<Equipe>   lstEquipe)
	 {
		 ...
		 for (int nEquipe = 1; nEquipe <= nbGroupe; nEquipe++) 
		 {
			 ...
			 else
			 {
				 ...
				 {
					 cptSalle++;
					 // verifie si il peut encore il y avoir des groupes
					 salle = lstInfoSalle.get(cptSalle).get(0);
					 lstInfoSalle.get(cptSalle).set(1, lstInfoSalle.get(cptSalle).get(1) - 1) ;
				 }
			 }
